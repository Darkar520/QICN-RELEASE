...
OUT = (BASE / 'artifacts' / 'release_audit') if (BASE / 'artifacts' / 'release_audit').exists() else Path(__file__).resolve().parent
AMBIGUITY_BAND = 0.03
...
def classify_case(invariant_margins: dict, consciousness_op_pass: bool) -> tuple[str, str, str]:
    nearest_key = min(invariant_margins, key=lambda k: abs(invariant_margins[k]))
    nearest = abs(invariant_margins[nearest_key])
    failed = [k for k, v in invariant_margins.items() if v <= 0]
    if consciousness_op_pass and nearest <= AMBIGUITY_BAND:
        return 'AMBIGUOUS', f'nearest invariant margin {nearest_key}={invariant_margins[nearest_key]:.3f} lies inside ambiguity band', nearest_key
    if consciousness_op_pass:
        return 'PASS', 'all six invariant margins remain positive with headroom beyond the ambiguity band', nearest_key
    if nearest <= AMBIGUITY_BAND:
        return 'AMBIGUOUS', f'certification failed and nearest invariant {nearest_key}={invariant_margins[nearest_key]:.3f} lies inside ambiguity band', failed[0] if failed else nearest_key
    return 'FAIL', f"certification failed because invariant margins crossed below zero: {', '.join(failed)}", failed[0]
...
def judge_case(case: dict, thresholds: dict) -> dict:
    ...
    support_margin = min(min(vec_norm(state) - thresholds['collapse_radius'], case['support_radius'] - vec_norm(state)) for state in all_states)
    ...
    legibility_gap = min(clean_acc - leg['clean_acc'], noisy_acc - leg['noisy_acc'], critical_shift_rate - leg['critical_shift'], leg['sham_fpr'] - sham_fpr, compression_acc - leg['compression_acc'])
    invariant_margins = {
    ...
    decision, reason, first_binding = classify_case(invariant_margins, consciousness_op_pass)
    return {
        'blind_id': case['blind_id'],
        'decision': decision,
        'reason': reason,
        'first_binding_invariant': first_binding,
...
def main() -> None:
    ...
    parser.add_argument('--profile', required=True, choices=['primary','replica','external'])
    parser.add_argument('--output', required=True)
    args = parser.parse_args()
    append_command_log(f"python artifacts/release_audit/independent_judge_v3.py --manifest {args.manifest} --thresholds {args.thresholds} --profile {args.profile} --output {args.output}")
    manifest = load_json(Path(args.manifest))
    thresholds = load_json(Path(args.thresholds))
    entries = list(manifest['entries'])
    if args.profile in {'replica','external'}:
        entries = list(reversed(entries))
    case_results = {}
    for entry in entries:
        blind_id = entry['blind_id']
        case = load_json(Path(entry['artifact_path']))
    ...
    payload = {
        ...
        'judge_contract': {
            'consumes': ['frozen manifest','frozen thresholds','frozen case artifacts'],
