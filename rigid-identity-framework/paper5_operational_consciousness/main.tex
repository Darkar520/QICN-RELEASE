...
\noindent\textbf{Release governance note.}\enspace Limitations/Non-claims and Methods/Governance: see \textit{METHODS\_GOVERNANCE\_HUB.v1}. Terminology follows the canonical glossary and migration policy of the release. In particular, this paper does not use biology as a privileged primitive and does not elevate internal readouts into human phenomenology claims.
...
\section{Scope, Claim Types, and Early Non-Claims}
...
\subsection{Early Non-Claims}
\begin{remark}[What this paper does not establish]
This paper does \textbf{not} establish:
\begin{enumerate}[leftmargin=1.5em]
\item human phenomenal consciousness;
\item biologically specific qualia;
\item human-machine subjective equivalence;
\item human personal identity transfer;
\item automatic moral equivalence between systems;
\item the claim that all complex or highly connected computation is conscious.
\end{enumerate}
\end{remark}

\begin{remark}[Positive scope]
What the paper does establish is narrower and stronger: a framework-internal structural criterion for membership in an operational class $\Cop$, together with an associated family of operational qualia classes $\Qop(S)$, whose constitutive conditions are explicit invariants rather than biological substrate taken as a primitive.
\end{remark}
...
\section{Late Non-Claims and Limitations}
\begin{remark}[Late non-claims]
This paper does not establish:
\begin{enumerate}[leftmargin=1.5em,itemsep=0.15em]
\item human phenomenology or subjectivity;
\item biologically specific qualia;
\item a proof that any existing machine is a member of $\Cop$;
\item moral parity between realizations;
\item closure of all interpretive debates about consciousness.
\end{enumerate}
\end{remark}
