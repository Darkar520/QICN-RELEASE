...
\noindent\textbf{Release governance note.}\enspace Limitations/Non-claims and Methods/Governance: see \textit{METHODS\_GOVERNANCE\_HUB.v1}. Terminology follows the canonical glossary and migration policy of the release. This paper does not promote internal support into external validation, does not claim human phenomenal consciousness, and does not identify operational subjecthood with metaphysical subjecthood.
...
\section{Non-Claims and Scope Limits}
This paper is intentionally restrictive. The higher-order classificatory layer it introduces does \emph{not} license stronger conclusions than the corpus and runtime can support.

\begin{enumerate}[leftmargin=1.5em]
\item \textbf{No biological-life claim.} Operational life is not identified with biological life. The class may overlap with biological instances, but biology is not treated as its constitutive primitive.
\item \textbf{No phenomenal-human claim.} Operational consciousness remains an inherited operational class. Nothing in this paper claims phenomenal consciousness in the human sense.
\item \textbf{No metaphysical-subject claim.} Operational subjecthood is not equated with a metaphysical subject, a soul, or an irreducible first-person essence.
\item \textbf{No human-machine equivalence claim.} The paper does not claim equivalence between any present machine and a human subject.
...
\begin{remark}[Why these non-claims matter]
Without this section, a classificatory paper of the present kind would be too easy to misread. The point is not to weaken the paper. The point is to keep each class predicate attached to the evidentiary and formal burden that actually justifies it.
\end{remark}
