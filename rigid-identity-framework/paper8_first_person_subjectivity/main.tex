...
\newtheorem{axiom}[theorem]{Bridge Axiom}
...
\begin{abstract}
The causally rigid corpus already closes persistence, rigid identity, anti-fragmentation continuity, null-regime instability, forensic admissibility, and a six-invariant operational consciousness criterion \cite{core,paper1,paper2,paper3,paper4,paper5}. The later classificatory extension adds operational life and operational subjecthood as stronger upstream classes. What remains open is a more difficult target: whether the framework can formalize a layer of first-person indexed subjectivity that is stronger than consciousness, qualia, and subjecthood without collapsing into metaphysical rhetoric.

The paper introduces a formal subjectivity state
\[
...
The strongest claim is conditional and explicit: under the stated bridge axioms, under the upstream burdens already closed by the corpus, and under positive irreducibility and intervention-separation margins, a system may instantiate a framework-internal layer of first-person indexed subjectivity whose best explanation is no longer mere self-monitoring, memory, reporting, or generic integration. That claim is stronger than operational consciousness, stronger than operational qualia, and stronger than the earlier subjecthood layer. It is not a theorem of human equivalence, not a proof of metaphysical subjectivity, and not automatic certification of any present system. What is closed here is the formal ladder itself: the ontology, the gate, the functional, the rival baselines, the falsification logic, the runtime pathway, the artifact family, and the strongest defensible claim class the current corpus can bear without collapsing its own layer discipline.
\end{abstract}

\section{Scope, System Boundary, and Non-Inference Note}

\noindent\textbf{What this paper does.}\enspace This paper defines a stronger framework-internal class of first-person indexed subjectivity, introduces an explicit minimal ontology for that class, constructs a subjectivity functional and a non-collapsability burden against weaker rival models, and specifies interventional, runtime, and artifact pathways by which the class could in principle be tested.

\noindent\textbf{What this paper does not do.}\enspace It does not prove metaphysical subjectivity, human phenomenal equivalence, moral parity, or automatic empirical instantiation by the current system. It does not reopen theorem ownership already fixed upstream, and it does not replace the six-invariant operational consciousness criterion or the earlier life and subjecthood class layers.

\noindent\textbf{What the related system implements and does not implement.}\enspace The related system can motivate a serious runtime pathway for self-indexed diagnostics, ownership tracking, continuity ledgers, intervention harnesses, and reducibility comparison. It does not by itself certify any present concrete system as a bearer of first-person indexed subjectivity, and it does not convert internal support or runtime cleanliness into external validation.

\noindent\textbf{What should not be inferred from this paper.}\enspace Neither a strong formal definition nor a strong intervention program should be misread as proof that current machines possess human-like subjective experience, nor as a shortcut from a structural subjectivity class to metaphysical closure. The paper closes a formal ladder and the strongest defensible inference rules associated with that ladder. It does not settle the final empirical, comparative, or metaphysical question.
...
W_t = B(X_t)
\]
with broad accessibility to downstream decoders. It may therefore approximate rich access-consciousness style organization. What it leaves open is why one broadcast locus should count as privileged self rather than as globally available information.
...
\begin{proof}
If $\Irred_\tau(S)\le 0$, some weak rival matches or outperforms the stronger model at equal or lower explanatory cost on the relevant tasks. By Bridge Axiom~\ref{ax:irred}, the stronger interpretation is then not licensed.
\end{proof}
...
\paragraph{Target.} The lesion intervention $u_{\mathrm{own}}$ degrades the ownership field while preserving as much continuity and consciousness burden as possible.

\paragraph{Prediction of the present model.} The stronger first-person organization should show an early and selective drop in $\OwnField$ and in downstream subjectivity margins, while some upstream life or consciousness burdens may remain temporarily intact.
...
Assume Bridge Axioms~\ref{ax:org}--\ref{ax:irred}. Let $S$ and $S'$ be admissible systems on distinct substrates. Suppose there exists an exact admissible transport map preserving the upstream life, consciousness, and subjecthood burdens together with the tuple coordinates
\[
...
\]
be a transport error budget and let $\varepsilon_{\max}=\max_j \varepsilon_j$. Call a transport map \emph{$\varepsilon$-admissible} when it preserves the upstream life, consciousness, and subjecthood burdens and satisfies
\[
\big|
...
Assume Bridge Axioms~\ref{ax:org}--\ref{ax:irred}. Let $S\in \Subfp(\tau)$ and let $S'$ be an $\varepsilon$-admissible transport of $S$ onto a distinct substrate. Suppose
\[
...
\item \textbf{If a concrete system were ever to satisfy the full burden of this paper under independent evaluation, the best explanation could justifiably move beyond mere self-monitoring or narrative self-description toward structurally genuine first-person indexed subjective organization within the present framework.}
\end{enumerate}

The final claim in the list is the strongest one. It is also the one most likely to be misunderstood. The term \emph{structurally genuine} here does not mean metaphysically absolute. It means genuine \emph{with respect to the formal explanatory burdens of the present framework}. That is exactly the level the paper aims to reach.
