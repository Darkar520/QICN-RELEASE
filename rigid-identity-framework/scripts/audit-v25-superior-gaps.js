...
const blocksOutcomeCopy =
  !/max\s*\(\s*pt\.noise_floor\s*,\s*pt\.observed_delta\s*\)/.test(runner)
  && runner.includes("pt.qicn_prediction")
  && (
    runner.includes("qicn_prediction exactly equal to observed_delta")
    || runner.includes("qicn_prediction near-equal to observed_delta")
    || runner.includes("v26 blocks near-copy outcome leakage")
  );
const hasSmallNTCritical =
  runner.includes("tCritical975")
  && runner.includes("8:2.306")
  && runner.includes("degrees_of_freedom");
const vetoProtocolBound =
  exists("docs/protocols/HUMAN_VETO_TRACEABILITY_PROTOCOL_v25.md")
  && (
    downgrade.includes("HUMAN_VETO_TRACEABILITY_PROTOCOL_v25.md")
    || downgrade.includes("HUMAN_VETO_SIGNATURE_PROTOCOL_v26.md")
  );
const semanticAuditExpanded =
  promo.includes("phenomenal character")
  && promo.includes("what it is like")
  && (
    promo.includes("sentence_and_paragraph")
    || promo.includes("sentence_level_and_adjacent_sentence_pair")
    || promo.includes("adjacent_sentence_pair")
  );
...
const checks = {
  ...
  no_tautological_predictor: status(blocksOutcomeCopy, "Outcome-copy predictor removed; exact or near-copy observed-delta leakage is blocked by the deployed runner.", "CLOSED_IMPLEMENTATION"),
  ...
  t_interval_present: status(hasSmallNTCritical, "t critical table used for small n; df-specific values including df=8 are present.", "CLOSED_IMPLEMENTATION"),
  ...
  human_veto_traceability: status(vetoProtocolBound, "Downgrade proposals point to a human veto protocol; v26 signature protocol is accepted as a forward-compatible successor to v25 traceability.", "CLOSED_PROTOCOL"),
  semantic_audit_expanded: status(semanticAuditExpanded, "Promotion audit lexicon expanded and scans sentence-level plus adjacent-sentence windows.", "PARTIAL_DETERMINISTIC_GATE"),
