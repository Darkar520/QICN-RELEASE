#!/usr/bin/env node
/*
 * QICN v30 Session Zero adjudicator — hardened.
 *
 * v30 closes five mathematical vulnerabilities identified in the v28 audit:
 ...
 * 4. --strict flag forces v30 logic; without it the runner falls back to
 *    v27 behavior so existing v26/v27 suites remain backward-compatible.
 * 5. When strict mode AND |ρ|>0.4, glsGaussianInformation computes the
 *    EXACT multivariate NLL under Prais-Winsten exact likelihood (not the
 *    approximate C-O transformation), and GLS AICc becomes the SOLE
 *    decision metric. A new blocking gate BLOCKED_AUTOCORRELATION_GLS_DETECTION
 *    fires when the GLS-corrected gain is negative (rival beats QICN under
 *    exact GLS).
 */
const fs = require("fs");
...
const { glsGaussianInformation } = require("./lib/gls-statistics");

const ROOT = path.resolve(__dirname, "..");
const DEFAULT_FIXTURE = path.join(ROOT, "docs", "fixtures", "EXTERNAL_SESSION_ZERO_SYNTHETIC_FIXTURE_v27.json");
const DEFAULT_REPORT = path.join(ROOT, "docs", "reports", "SESSION_ZERO_SYNTHETIC_FIXTURE_ADJUDICATION_v30.json");
const GOVERNANCE = "This v30 adjudication report is an internal diagnostic gate implementing exact GLS multivariate likelihood. It does not certify external support, consciousness, phenomenality, identity transfer, bridge-burden closure, or human mathematical review.";
...
// Ljung-Box needs lags beyond one; estimateRho is the canonical lag-1 gate.
function lagAutocorrelation(residuals, lag) {
...
function ljungBox(residuals, maxLag = Math.min(3, residuals.length - 1)) {
  ...
  for (let lag = 1; lag <= maxLag; lag += 1) {
    const rho = lagAutocorrelation(residuals, lag);
...
function analyzeManifest(manifest, options = {}) {
  ...
  const legacyV27 = options.legacyV27 === true;
  ...
  const qicnGls = glsGaussianInformation(points, "qicn_prediction", validation.paramCounts.qicnK, penalty);
  const rivalGls = glsGaussianInformation(points, "rival_prediction", validation.paramCounts.rivalK, penalty);

  let gainAiccIid = rivalIid.aicc - qicnIid.aicc;
  let gainAiccAr1 = rivalAr1.aicc - qicnAr1.aicc;
  let gainAiccGls = rivalGls.aicc - qicnGls.aicc;
  ...
  } else if (strict && autocorrelationDetected) {
    decisionGainAicc = gainAiccGls;
    decisionMethod = "gls_exact_aicc_mandatory_when_rho_exceeds_" + RHO_BLOCK_THRESHOLD;
  } else if (strict) {
  ...
  if (strict && dw !== null && dw < 0.5) {
    console.warn(`[v30 STRICT] Fixture DW=${dw.toFixed(4)} < 0.5 — severe serial autocorrelation. BLOCKED.`);
  } else if (dw !== null && dw < 0.5) {
    console.warn(`[v30 WARNING] Fixture DW=${dw.toFixed(4)} < 0.5 — severe serial autocorrelation in synthetic fixture. This fixture would be BLOCKED as external data.`);
  }
  ...
  if (strict && autocorrelationDetected && (rivalGls.aicc - qicnGls.aicc) < 0) {
    blockingReasons.push("BLOCKED_AUTOCORRELATION_GLS_DETECTION");
  }
  ...
  let modelComparison = {
    ...
    decision_basis: strict && autocorrelationDetected
      ? "GLS-exact AICc is the sole decision metric when |rho|>" + RHO_BLOCK_THRESHOLD + ". AR(1) C-O corrected AICc and iid-AICc are reported as diagnostics but do not control the verdict."
  ...
  if (legacyV27) {
    modelComparison = {
      method: decisionMethod,
      decision_basis: "iid-AICc controls the support threshold (legacy v27 mode, GLS and AR(1) corrections skipped).",
      qicn_iid_diagnostic_only: qicnIid,
      rival_iid_diagnostic_only: rivalIid,
      iid_gain_aicc_biased_diagnostic: gainAiccIid,
      decision_gain_aicc: decisionGainAicc,
  ...
  return {
    ...
    temporal_dependence_diagnostic: {
      ...
      durbin_watson_policy: strict
        ? "block_on_violation_for_all_data_synthetic_and_external_strict_v30"
    ...
    external_support_certified: false,
    nonclaim_boundary: "A v30 PASS here means the adjudication gate executed and enforced its blockers using exact GLS multivariate likelihood; it is not evidence for QICN, consciousness, identity transfer, phenomenality, or bridge closure."
  };
}
...
function main() {
  ...
  const strict = !legacyV27;
  const report = adjudicateFile(manifestPath, DEFAULT_REPORT, { strict, legacyV27 });
  console.log(`External Session Zero adjudicator v30: ${report.result}; verdict=${report.verdict}; strict=${report.strict_mode}; legacy_v27=${report.legacy_v27_mode}; blockers=${report.blocking_reasons.length}; external_support_certified=false`);
